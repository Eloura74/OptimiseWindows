...
namespace lsaa {
    ...
    public:
        ...
        void endFrame() {
            ...
            glClearColor(0.10f, 0.11f, 0.13f, 1.0f); // Match BG
        ...
        void drawUI(double cpu, long long ramUsed, long long ramTotal, 
             ...
             // Main Docking - Fullscreen
             const ImGuiViewport* viewport = ImGui::GetMainViewport();
             ImGui::SetNextWindowPos(viewport->WorkPos);
             ImGui::SetNextWindowSize(viewport->WorkSize);
             // ImGui::SetNextWindowViewport(viewport->ID); // Removed: Not available in this ImGui version
             ...
             if (ImGui::BeginTabBar("MainTabs", ImGuiTabBarFlags_None)) {
                 ...
                 // TAB 2: RULES EDITOR
                 if (ImGui::BeginTabItem(" RULES CONFIG ")) {
                     renderRulesEditor();
                     ImGui::EndTabItem();
                 }
    ...
    private:
        ...
        void renderDashboard(double cpu, long long ramUsed, long long ramTotal, 
                          ...
                          std::function<void()> onSendNotification) 
        {
             // Use Child Windows for Layout to mimic the previous look within the tab
             float width = ImGui::GetContentRegionAvail().x;
             ...
             ImGui::BeginChild("LeftCol", ImVec2(col1W, height), false);
                ...
                ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.7f, 1.0f), "MEMORY USAGE (%.2f GB)", ramUsed / 1024.0 / 1024.0 / 1024.0);
                ...
                if (ImGui::Button("RUN CLEAN SCRIPT", ImVec2(-1, 40))) if (onRunScript) onRunScript();
                ImGui::Spacing();
                if (ImGui::Button("TEST NOTIFICATION", ImVec2(-1, 40))) if (onSendNotification) onSendNotification();
                ImGui::EndChild();
        ...
        void renderRulesEditor() {
            ...
            if (ImGui::BeginTable("RulesTable", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
                ...
                ImGui::TableSetupColumn("Threshold Limit");
        ...
        void renderOptimizer() {
            ...
            ImGui::Spacing();
            if (ImGui::Button("SCAN NOW", ImVec2(200, 50))) {
                lastScan_ = cleaner_->scan();
            }

            ImGui::Spacing();
            if (lastScan_.fileCount > 0) {
                 ImGui::TextColored(ImVec4(1, 0.8f, 0, 1), "Found %zu files taking up %.2f MB", 
                    lastScan_.fileCount, lastScan_.totalSize / 1024.0 / 1024.0);
                 
                 ImGui::Spacing();
                 if (ImGui::Button("CLEAN ALL", ImVec2(200, 50))) {
                     cleaner_->clean();
                     lastScan_ = cleaner_->scan(); // Rescan
                 }
            } else {
                ImGui::TextDisabled("No junk files found or scan not run.");
            }
        }
